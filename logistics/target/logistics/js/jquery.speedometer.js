...
Usage: 
	$jj([selector]).speedometer([options object]);
...
( function( $ ){
	...
	$.fn.speedometer = function( options ){
		...
		/* A tad bit speedier, plus avoids possible confusion with other $jj(this) references. */
		var $this = $( this );
		...
		if ( def.limit && ( def.percentage > def.maximum || def.percentage < def.minimum ) ) {
			/* the glass cracks */
			$this.css( 'backgroundImage' , 'url("./images/background-broken.jpg")' );
		} else {
		...
		var decision_result = $("#decision_result").val();
		if(decision_result == "Reject"){
			digitalGauge.text("高风险");
			digitalGauge.css("color","red");
		}else if(decision_result == "Approve"){
			digitalGauge.text("低风险");
			digitalGauge.css("color","green");
		}else if(decision_result == "Decision"){
			digitalGauge.text("待评估");
			digitalGauge.css("color","yellow");
		}else{
			digitalGauge.text("");
		}
